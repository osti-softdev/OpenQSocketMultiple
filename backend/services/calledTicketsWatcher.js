...
async function updateCalledTicket(id, counterHist) {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.serialize(() => {
            ...
            db.get(`SELECT sname, counter_num, counter_user FROM transactions WHERE id = ?`, [id], (err, row) => {
                ...
                // 3. Finish previous "called" ticket of same sname for this counter
                const finishPrev = `
                    UPDATE transactions
                    SET status = 'finished',
                        end_time = ?,
                ...
                db.run(finishPrev, [time, finishEntry, finishEntry, sname, cname, cnum, id], function (err) {
                    if (err) {
                        db.run("ROLLBACK");
                        db.close();
                        return reject(err);
...
// --- Setup all watchers ---
async function setupCalledTicketsWatcher(socket, io) {
		...
		socket.on("updateCalledTicket", async (data) => {
			...
			try {
				const updated = await updateCalledTicket(id, counter_hist);
				if (updated) {
