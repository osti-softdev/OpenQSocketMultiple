...
async function handleKey(key, io) {
	...
	const topline = "Topline"; // change if you want dynamic
	...
	try {
		...
		// === Call next ticket (A,B,C,D) ===
           ...
           WHERE id = ?`,
					[startTime, historyEntry, historyEntry, pending.id]
				);
		...
		// === Recalling (#) ===
         ...
         ORDER BY start_time DESC LIMIT 1`,
			...
			if (!calling) {
				db.close();
				return { ok: false, error: "NO_CALLING" };
			}
         ...
         WHERE id = ?`,
			...
			db.close();
			io.emit("ticket:recall", row);
			return { ok: true, action: "RECALL", row };
		}
		...
		// === Voided (0) ===
         ...
         ORDER BY start_time DESC LIMIT 1`,
			...
			await dbRun(
				db,
				`UPDATE transactions 
				SET status = 'voided', end_time = ?,
					history = CASE WHEN history IS NULL OR history = '' THEN ? ELSE history || ';' || ? END
				WHERE id = ?`,
				[time,historyEntry, historyEntry, calling.id]
			);
			...
			db.close();
			io.emit("ticket:voided", row);
			return { ok: true, action: "VOIDED", row };
		}
