...
function dbRun(db, sql, params = []) {
	return new Promise((resolve, reject) => {
		db.run(sql, params, function (err) {
			...
			// return lastID / changes if needed
...
function setupKeyApi(io) {
	...
	// NOTE: Ensure express.json() is used in index.js BEFORE mounting this router.
	// POST: { "key": "2" }
	// GET:  /api/key/2
...
async function handleKey(key, io) {
	...
	try {
		...
		if (["2", "3", "4"].includes(key)) {
			...
			// get current max ticket for that service/date
		...
		// === Call next ticket (A,B,C,D) ===
			...
			if (key === "A") {
				// global: next pending (any service)
				// find pending id
				const pending = await dbGet(
					db,
					`SELECT id FROM transactions WHERE status = 'pending' AND date = ? ORDER BY date ASC, time ASC LIMIT 1`,
					[date]
				);
				if (!pending) {
					db.close();
				...
				// read the updated row to return
				const row = await dbGet(
					db,
				...
				// B/C/D are service-specific
				const services = await getAllServices();
				// map key -> service index: B->2, C->3, D->4 then -2 to index into array
				...
				// find pending id for that service
				const pending = await dbGet(
					db,
					`SELECT id FROM transactions WHERE status = 'pending' AND sname = ? AND ticketservice = ? AND date = ? ORDER BY date ASC, time ASC LIMIT 1`,
					[service.sname, service.regular, date]
				);
				if (!pending) {
					db.close();
		...
		// === Revert last called ticket (#) ===
			...
			// find most recent called id
			const called = await dbGet(
				db,
				`SELECT id FROM transactions WHERE status = 'called' AND date = ? ORDER BY start_time DESC LIMIT 1`,
				[date]
			);
			if (!called) {
				db.close();
