$(document).ready(function () {
	...
	socket.on("dashadmindataupdate", (adminoveralldata) => {
		...
		if (!barChartContent2) {
			...
			barChartContent2 = new Chart(ctx, {
				...
				data: {
					...
					datasets: [
						{
							label: "Services",
							data: calledCounts.concat([null, null]), // align with feedback slots
							backgroundColor: serviceBackgrounds.concat([null, null]),
				...
				options: {
					...
					plugins: {
						legend: { display: false, position: "right" }, // ✅ keep visible + ordered
						title: {
							display: true,
							text: "Services and Feedback Overview",
							color: "black"
						},
						datalabels: {
							color: "black",
			...
			barChartContent2.data.datasets[0].data = calledCounts.concat([null, null]);
			barChartContent2.data.datasets[0].backgroundColor = serviceBackgrounds.concat([null, null]);
	...
	socket.on("dashadmincontent2dataaverages", (data) => {
		...
		if (!data.transactions || data.transactions.length === 0) {
			...
			data.transactions.forEach(tr => {
				$container.append(`
					<div class="metric">
						<span>${tr.date}:</span> Total: ${tr.total_transactions}, Avg/Date: ${Math.round(tr.average_count_per_date)}
					</div>
				`);
			});
		}
