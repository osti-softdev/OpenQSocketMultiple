                ...
                callback({
                    ...
                    data: res.data.map(row => {
                        return [
                            ...
                            row.avg_turnaround_time || "00:00:00",
...
function initServiceTable(tableId) {
    if ($.fn.DataTable.isDataTable(tableId)) {
        $(tableId).DataTable().clear().destroy();
    }
    ...
    return $(tableId).DataTable({
        ...
        responsive: true,
        language: { search: "Search records:" },
        order: [[0, 'asc']], // default order by sname

        ajax: function (data, callback) {
            const datefrom = $('#startDate').val();
            const dateto = $('#endDate').val();

            socket.emit("requestAdminDataforcontent3services", {
                start: data.start,
                length: data.length,
                order: data.order,
                search: data.search.value,
            ...
            socket.once("dashadmincontent3services", function (res) {
                callback({
                    draw: data.draw,
                    recordsTotal: res.recordsTotal,
                    recordsFiltered: res.recordsFiltered,
                    data: res.data.map(row => {
                        ...
                        return [
                            ...
                            row.avg_turnaround_time || "00:00:00",
                            row.avg_waiting_time || "00:00:00",
                            `<button class="btn btn-sm btn-info view-service-details" 
                                     data-sname="${row.sname}">
                                     View Details
                                </button>`
                        ];
...
$(document).on('click', '.view-history', function () {
    ...
    const isFromService = $(this).data('from-service');
    ...
    Swal.fire({
        ...
        } else if (isFromService && window.currentServiceDetailsSwal) {
            Swal.fire(window.currentServiceDetailsSwal);
        }
    });
});
...
// 🔹 Delegate view service details clicks
$('#adminTable3').on('click', '.view-service-details', function () {
    const sname = $(this).data('sname');
    const datefrom = $('#startDate').val();
    const dateto = $('#endDate').val();

    socket.emit("requestServiceDetails", { sname, datefrom, dateto });

    socket.once("serviceDetailsData", function (res) {
        const chartData = res.chartData || [];
        const pieData = res.pieData || [];
        const historyRows = res.historyRows || [];
        ...
        const displaySname = sname?.replace(/_/g, " ") || "";

        let html = `
                <div style="font-family: 'Inter', sans-serif;">
                    <div style="text-align:center; margin-bottom:20px; padding: 15px; background: linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%); border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.05);">
                       <h3 style="margin:0; font-size:22px; color: #2c3e50; font-weight:700;">
                          Service: <span style="color: #2980b9;">${displaySname}</span> 
                       </h3>
                    </div>
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 20px; justify-content: center; margin-bottom: 25px;">
                        <div style="flex: 1 1 55%; min-width:300px; height: 280px; background: white; padding: 15px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border: 1px solid #eee;">
                            <h4 style="margin-top:0; font-size:14px; color:#7f8c8d; text-align:left; margin-bottom:10px;">Performance Overview</h4>
                            <div style="height: 230px;"><canvas id="serviceDetailsChart"></canvas></div>
                        </div>
                        <div style="flex: 1 1 40%; min-width:250px; height: 280px; background: white; padding: 15px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border: 1px solid #eee;">
                            <h4 style="margin-top:0; font-size:14px; color:#7f8c8d; text-align:left; margin-bottom:10px;">Tellers Handled</h4>
                            <div style="height: 230px; position:relative;"><canvas id="servicePieChart"></canvas></div>
                        </div>
                    </div>
        ...
        const swalConfig = {
            ...
            didOpen: () => {
                ...
                const pieLabels = pieData.map(d => d.teller);
                const pieCounts = pieData.map(d => d.count);
        ...
        // Store globally so history view can restore it
        window.currentServiceDetailsSwal = swalConfig;
        Swal.fire(swalConfig);
    });
});
